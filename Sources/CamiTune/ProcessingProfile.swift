...
struct ProcessingProfile: Codable, Hashable, Sendable {
    ...
    static func toneStageID(forChannel index: Int) -> UUID {
        let value = UInt32(clamping: index)
        return UUID(uuid: (
            0x43, 0x41, 0x4d, 0x49, 0x54, 0x55, 0x4e, 0x45,
            0x54, 0x4e, 0x00, 0x00,
...
struct ProcessingChain: Codable, Hashable, Sendable {
    ...
    init(stages: [ProcessingStage] = [], simpleTone: SimpleToneSettings? = nil) {
        self.stages = stages
...
extension ProcessingChain {
    var channelSettings: ChannelProcessingSettings {
...
extension ProcessingProfile {
    func settings(forGroup id: SpeakerGroupID) -> ChannelProcessingSettings? {
        groups.first { $0.id == id }?.chain.channelSettings
    }

    mutating func setGroupProcessing(id: SpeakerGroupID, settings: ChannelProcessingSettings) {
